...
def test_create(app, ingredients, beverages, size, client_data):
    ...
    for param, value in order.items():
        ...
        ingredients_in_detail = set(item['ingredient']['_id'] for item in created_order['detail_ingredient'])
        beverages_in_detail = set(item['beverage']['_id'] for item in created_order['detail_beverage'])
...
def test_calculate_order_price(app, ingredients, beverages, size, client_data):
    ...
    pytest.assume(created_order['total_price'] == round(created_size['price'] + sum(ingredient['price'] for ingredient in created_ingredients) + sum(beverages['price'] for beverages in created_beverages), 2))
